...
def depthFirstSearch(problem):
    ...
    explored = []
    while not problem.isGoalState(s[0]):
        l = problem.getSuccessors(s[0])
        for item in l: 
            if item[0] not in explored: 
                box.push((item[0],s[1]+[item[1]]))
        if box.isEmpty(): return 'failure'
        explored.append(s[0])
        y = 0
        while y==0:
            x = box.pop()
...
def uniformCostSearch(problem):
    ...
    while not problem.isGoalState(s[0]):
        ...
        for item in l: 
            if item[0] not in explored:  
                box.push((item[0],s[1]+[item[1]],s[2]+item[2]),s[2]+item[2])
        if box.isEmpty(): return 'failure'
        explored.append(s[0])
        y = 0
        while y==0:
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not problem.isGoalState(s[0]):
        ...
        for item in l: 
            if item[0] not in explored:
                priority = s[2]+item[2]+heuristic(item[0],problem) 
                box.push((item[0],s[1]+[item[1]],s[2]+item[2]),priority)
        if box.isEmpty(): return 'failure'
        explored.append(s[0])
        y = 0
        while y==0:
